...
r = robjects.r
logger = logging.getLogger('pfred.scripts.siRNA_descriptors')
...
####################################
# NOTE: AFAIK, this just drop columns where values are the same
# but I am just abiding to the original implementation in case
# I'm missing something
def stdev_filter(raw_dict,new_dict):
    # keys = raw_dict.keys()
    # var2remove = []
    # for i in range(len(raw_dict[list(keys)[0]])):
    #     tmp_var_i = []
    ...
    # if ((columns_to_remove == False).all()):
    #     logger.warning("Nothing is removed")
    # else:
    #     logger.warning("Something is removed")
...
def thermodynamic(dict_sequences):
    ...
    ki = list(Thermo_descriptors.keys())
