...
logger = logging.getLogger("pfred.scripts.siRNA_stats")

def pls_regression_cross_validation(TRAINpls):
    model = r("activity ~ .")
    PLSfit = r.plsr(model,20,data=TRAINpls, scale=True)
    
    PLSval = r.plsr(model,20,data=TRAINpls,validation="CV", scale=True)
    r.summary(PLSval)
...
def pls_regression_ext_validation(TRAINpls,TESTpls,activity_dict,NAMES,output_type):	
    model = r("activity ~ .")
    PLSfit = r.plsr(model,10,data=TRAINpls, scale=True)
...
def svm_regression(TRAINsvm,TESTsvm,activity_dict,output_type):
    formula = r("activity ~ .")
    SVMfit = r.svm(formula,data=TRAINsvm, scale=True)
    SVMself = r.predict(SVMfit,TRAINsvm)
    SVMpred = r.predict(SVMfit,TESTsvm)
    #for obj in SVMfit:
...
def rf_regression(TRAINrf,TESTrf,activity_dict,output_type):
    model = r("activity ~ .")
    RFmod = r.randomForest(model,data=TRAINrf,xtest=TESTrf,scale=True,ntree=1000) #5000 is more reliable but it takes longer
    if output_type == "build":
